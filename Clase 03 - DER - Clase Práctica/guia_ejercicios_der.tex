...
\begin{tikzpicture}[node distance=2cm]
    ...
    \coordinate (fk_materia) at ($(dictado.north)+(0,0.1)$);
    \coordinate (pk_materia) at ($(materia.south)+(0,-0.1)$);
    \draw[-latex] (fk_materia) to[out=90,in=-90] 
        node[pos=0.2,right] {1..N} 
        node[pos=0.8,right] {1..1} 
        (pk_materia);
        
    % Conexiones dictado-inscripcion
    \coordinate (fk_dictado) at ($(inscripcion.north)+(0,0.1)$);
    \coordinate (pk_dictado) at ($(dictado.south)+(0,-0.1)$);
    \draw[-latex] (fk_dictado) to[out=90,in=-90] 
        node[pos=0.2,right] {1..N} 
        node[pos=0.8,right] {1..1} 
        (pk_dictado);
    ...
    \coordinate (pk_alumno) at ($(alumno.south)+(0,-0.1)$);
...
\begin{tikzpicture}[node distance=2cm]
    ...
    \coordinate (fk_proyecto_asignacion) at ($(asignacion.north)+(0,0.1)$);
    \coordinate (pk_proyecto_asignacion) at ($(proyecto.south)+(0,-0.1)$);
    \draw[-latex] (fk_proyecto_asignacion) to[out=90,in=-90] 
        node[pos=0.2,right] {1..N} 
        node[pos=0.8,right] {1..1} 
    ...
    \coordinate (fk_tarea_dependiente) at ($(dependencia.north)+(0,0.1)$);
    \coordinate (pk_tarea_dependiente) at ($(tarea.south)+(0,-0.1)$);
    \draw[-latex] (fk_tarea_dependiente) to[out=90,in=-90] 
        node[pos=0.2,right] {1..N} 
        node[pos=0.8,right] {1..1} 
    ...
    \coordinate (pk_tarea_requerida) at ($(tarea.south)+(0,-0.1)$);
    \draw[-latex] (fk_tarea_requerida) to[out=0,in=180] 
        node[pos=0.2,above] {N..1} 
        node[pos=0.8,above] {1..1} 
        (pk_tarea_requerida);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=2cm]
    ...
    \coordinate (fk_medico) at ($(consulta.north)+(0,0.1)$);
    \coordinate (pk_medico) at ($(medico.south)+(0,-0.1)$);
    \draw[-latex] (fk_medico) to[out=90,in=-90] 
        node[pos=0.2,right] {1..N} 
        node[pos=0.8,right] {1..1} 
        (pk_medico);
        
    % Conexiones consulta-receta
    \coordinate (fk_consulta) at ($(receta.north)+(0,0.1)$);
    \coordinate (pk_consulta) at ($(consulta.south)+(0,-0.1)$);
    \draw[-latex] (fk_consulta) to[out=90,in=-90] 
        node[pos=0.2,right] {1..N} 
        node[pos=0.8,right] {1..1} 
        (pk_consulta);
...
\begin{tikzpicture}[node distance=2cm]
    ...
    \node[entity] (perfil) [right=of usuario] {Perfil\nodepart{second}\textbf{id\_perfil (PK)}\\ \underline{email\_usuario (FK)}\\ nombre\\ fecha\_creacion};
    \node[entity] (contenido) [right=of perfil] {Contenido\nodepart{second}\textbf{id\_contenido (PK)}\\ titulo\\ genero\\ anio\\ clasificacion\\ duracion};
    \node[entity] (reseña) [below=of contenido] {Reseña\nodepart{second}\textbf{id\_reseña (PK)}\\ \underline{id\_perfil (FK)}\\ \underline{id\_contenido (FK)}\\ puntuacion\\ comentario};
    
    % Conexiones usuario-perfil
    ...
    \coordinate (fk_contenido) at ($(reseña.north)+(0,0.1)$);
    \coordinate (pk_contenido) at ($(contenido.south)+(0,-0.1)$);
    \draw[-latex] (fk_contenido) to[out=90,in=-90] 
        node[pos=0.2,right] {1..N} 
        node[pos=0.8,right] {1..1} 
        (pk_contenido);
        
    % Conexiones perfil-reseña
    \coordinate (fk_perfil_reseña) at ($(reseña.west)+(0,-0.5)$);
    \coordinate (pk_perfil_reseña) at ($(perfil.south)+(0,-0.1)$);
    \draw[-latex] (fk_perfil_reseña) to[out=0,in=180] 
        node[pos=0.2,above] {N..1} 
        node[pos=0.8,above] {1..1} 
        (pk_perfil_reseña);
\end{tikzpicture}
